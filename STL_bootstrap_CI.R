# This code reads the kaggle data for STL and performs bootstrap confidence intervals for AUC using 
# the optimism method
...
for (t in 1:K)
{
  ...
  result <- BCa(XYt,nboot=1000, function(x){RunInnerBootstrap(x,B=200,LASSO=TRUE)},quiet=F)
  auc_CI[t,] = result
  etm = proc.time()
